...
IMPORTANT: Only identify strengths, arguments, and rebuttals that ACTUALLY appear in the transcript. Do not invent or fabricate anything. If a side has no clear strengths, state that honestly. If there is no clear best argument or rebuttal, state that honestly."""))
        ...
        # DEBUG: Log RAW judge response
        print("=" * 80)
        print(f"DEBUG: RAW Judge Response (game_mode={game_mode}):")
        print("=" * 80)
        print(verdict_text)
        print("=" * 80)
        ...
        for i, line in enumerate(lines):
            ...
            if len(st.session_state.messages) >= 2:
                # Get the last message as a potential rebuttal
                last_msg = st.session_state.messages[-1]
                if last_msg.content:
                    verdict['best_rebuttal'] = last_msg.content[:200] + "..." if len(last_msg.content) > 200 else last_msg.content
                    print(f"DEBUG: Using fallback best_rebuttal from last message: {verdict['best_rebuttal']}")
            else:
        ...
        if game_mode == 'human_vs_ai':
            ...
            # Override winner calculation based on actual scores for Human vs AI
            human_score = st.session_state.pro_score if user_side == 'pro' else st.session_state.con_score
            ai_score = st.session_state.con_score if user_side == 'pro' else st.session_state.pro_score
        ...
        # Store Judge result in session state based on game mode
        if game_mode == 'ai_vs_ai':
            st.session_state["ai_vs_ai_judge_result"] = verdict.copy()
        elif game_mode == 'human_vs_ai':
            st.session_state["human_vs_ai_judge_result"] = verdict.copy()
        
        # DEBUG: Log final best_rebuttal value before returning
        print(f"DEBUG: Final best_rebuttal value before returning: {verdict['best_rebuttal']}")
        print("=" * 80)
...
def render_verdict_page():
    ...
    elif game_mode == 'human_vs_ai' and 'human_vs_ai_judge_result' in st.session_state:
        verdict = st.session_state['human_vs_ai_judge_result']
    else:
        verdict = st.session_state.judge_verdict
